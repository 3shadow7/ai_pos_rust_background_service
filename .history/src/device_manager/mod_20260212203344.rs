...
use crate::hardware::printer::{MockPrinter, network::NetworkPrinter, serial::SerialPrinter};
        ...
        // Load Printers
        {
            ...
            for p_conf in &config.printers {
                let printer: Arc<dyn Printer> = match p_conf.device_type.as_str() {
                    ...
                    "serial" => {
                        // Default baud rate 9600 if not specified (config structure might need update for baud, defaulting for now)
                        let baud = 9600; 
                        Arc::new(SerialPrinter::new(p_conf.id.clone(), p_conf.connection.clone(), baud))
                    },
                    _ => {
