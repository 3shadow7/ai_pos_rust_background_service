...
impl WindowsPrinter {
    ...
    fn send_raw_to_printer(&self, data: &[u8]) -> Result<(), ServiceError> {
        ...
        unsafe {
            ...
            // Log success open
            // info!("Opened printer handle: {:?}", h_printer);

            // Guard to ensure we ClosePrinter even if we crash/return early below
            let _guard = PrinterHandleGuard(h_printer);

            // 2. Start Document
            let doc_name: Vec<u16> = "POS Service Receipt".encode_utf16().chain(std::iter::once(0)).collect();
            ...
            if !success {
                ...
                // info!("Bytes written to printer: {}", bytes_written);
            }
...
impl Printer for WindowsPrinter {
    ...
    async fn print_raw(&self, data: &[u8]) -> Result<(), ServiceError> {
        ...
        .map_err(|e| ServiceError::DeviceError(format!("Thread error: {}", e)))??;
