...
impl WindowsPrinter {
    ...
    fn send_raw_to_printer(&self, data: &[u8]) -> Result<(), ServiceError> {
        ...
        unsafe {
            ...
            // OpenPrinterW takes PWSTR (mutable pointer)
            let mut printer_name_wide: Vec<u16> = self.printer_name.encode_utf16().chain(std::iter::once(0)).collect();
            let p_printer_name = PWSTR(printer_name_wide.as_mut_ptr());

            // 1. Open Printer
            if !OpenPrinterW(p_printer_name, &mut h_printer, None).as_bool() {
            ...
            if StartDocPrinterW(h_printer, 1, &mut doc_info as *mut _ as *mut u8) == 0 {
                let err = std::io::Error::last_os_error();
