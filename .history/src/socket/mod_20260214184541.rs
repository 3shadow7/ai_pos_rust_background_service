    ...
    match command {
        ...
        Ok(Command::Print { device_id, data }) => {
            if let Some(printer) = devices.get_printer(&device_id).await {
                // 1. Print Text (if provided)
                if let Some(text) = data.text {
                     if let Err(e) = printer.print_text(&text).await {
                         return Response { status: "error".into(), device_id: Some(device_id), message: Some(e.to_string()) };
                     }
                }
                
                // 2. Auto-Cut (if requested)
                if data.auto_cut {
                    if let Err(e) = printer.cut_paper().await {
                         return Response { status: "error".into(), device_id: Some(device_id), message: Some(format!("Print ok, but cut failed: {}", e)) };
                    }
                }
